...
![橙色电车沿弯轨前进，街区从线稿、纸模型过渡到暖灯照亮的写实建筑。](assets/seaimagine-seedance-hero-v5.jpg)

以原创场景表现从结构草图到材质、光照与运动的创作构想。

这里收录 **120 条完整视频提示词、12 个社区视频案例和 9 个官方演示**。从产品广告、电影短片到纸艺动画、旅行记录，选一个喜欢的场景，读懂写法，再到 SeaImagine 试着做出来。

主库包含 60 条中文和 60 条英文提示词，另有 14 份语言练习，每份包含 6 个场景。15 种语言首页方便你选择入口；下方 12 个社区案例都配有中文解读和可复制的改写提示词。
...
| 已有产品图、人物照或插画 | [九条中文示例](#精选原创提示词)，前三条配有可直接使用的参考图 |
...
看预览、读作者原文，再展开提示词动手改写。下面 12 个案例分别展示声音、运镜、人物互动和场景衔接的写法。
...
**输入素材：**文字描述；虚构人物。下方改写将场景调整为成年人的特技训练。 **来源规格：**来源提示词要求 30 秒；上传视频约 30.17 秒，1280×720。
...
**输入素材：**来源要求提供一张女性参考图；选择人物外貌清晰的人像。 **来源规格：**来源提示词要求 30 秒；上传视频约 30.08 秒，1280×720。
...
从后台登台、乐团演出到早餐运镜，先观察画面，再用完整练习提示词拆解人物、空间和动作。每例还附有一个换场景练习，方便你把同一种写法用到自己的故事里。
...
```text
生成一段 30 秒、16:9 的写实演出短片，用一个连续跟拍镜头表现同一位原创女歌手从化妆间走上舞台。场景依次相连，后台是暖色实用灯，舞台是红黑配色；歌手的脸、卷发、深绿色演出服和耳返保持一致。

0–6 秒：从化妆镜侧面中景开始，歌手确认耳返位置，工作人员在门口给出准备上场的手势。镜头缓慢后退，为她起身和转身留出空间，不从镜子中制造第二个人物。
6–13 秒：她穿过门口进入走廊，镜头保持人物半身大小，沿她前方平稳后退。工作人员从侧面递来话筒，她明确伸手接住；话筒只交接一次，始终能看清持有者。
13–21 秒：两位伴舞在走廊尽头汇合，留出通道，三人沿同一路线走向舞台。镜头通过门框和光线变化交代空间转换，不靠黑场或剪切跳过行走。
21–27 秒：歌手进入聚光灯范围，伴舞分立两侧。镜头从她侧面沿宽弧线绕至斜后方，露出演出空间；灯光逐渐增强，人物轮廓仍然清晰。
27–30 秒：镜头缓慢后退，停在能同时看见歌手、舞台和观众区的全景。歌手面向观众站稳，完成开场姿态。

声音：脚步、衣料、接话筒的轻响依次出现，随着进入舞台，远处掌声逐渐变清晰；采用原创器乐铺底，不使用现有歌曲或模仿歌手声线。无对白、字幕、可辨识商标。保持动作有重量、手指接触合理，不改变服装、不增生话筒、不穿墙、不瞬移。
```
...
```text
制作 30 秒、16:9 的原创室内乐演出短片。使用自己有权使用的六张参考图：图 1 定义音乐厅结构与座位；图 2 定义主唱外貌及服装；图 3 定义钢琴师与钢琴；图 4 定义弦乐组；图 5 定义合唱团；图 6 定义观众的服装与座席氛围。每图只负责指定对象，不混入其他图片的背景。当前入口必须支持分别指定六张参考图，否则先减少参考数量并重写分工。

固定布置：主唱在舞台前方中央，钢琴在画面左侧，弦乐在右侧，合唱团站在后方阶梯；所有人物和乐器在全片保持身份、数量及位置关系。暖金色顶光，真实音乐厅材质，不加入漂浮灯带或装饰文字。
0–6 秒：从观众席后方的高位全景缓慢推近，建立舞台与座位的空间关系，演奏者准备就绪。
6–12 秒：切至钢琴师侧面中景，手指按键与原创钢琴声同步，主唱在背景站稳，不突然换位。
12–18 秒：切至弦乐组，镜头缓慢横移，琴弓朝向一致，弓毛与琴弦接触，不穿过乐器。
18–25 秒：回到主唱和后方合唱团的中远景，共同唱出无词元音和声，嘴部动作与声音匹配，不生成歌词或字幕。
25–30 秒：在同一机位缓慢拉远，演奏结束后人物落手，观众随后鼓掌。掌声不能早于最后一个乐音。

声音有适度厅堂混响，钢琴、弦乐、人声层次清楚，不调用已有曲目或指定真人歌声。剪切前后空间方向一致，不互换乐器、不让观众出现在舞台、不额外复制演员。
```

**看画面学什么：** 对照画面中的舞台布局与不同表演者，理解多参考要分别控制哪些主体。
...
```text
编辑一段 15 秒的原创双人早餐视频，只改机位与摄影机运动。输入视频应包含做早餐、煎蛋翻动及人物整理餐桌的连续动作；先确认原片动作发生的时点，再将下面分段对齐原片。保持原片人物外貌、服装、动作顺序、厨具数量、厨房布局、画风与原声音轨，不新增表演或配乐。

0–4 秒：镜头由料理台边缘低机位缓慢向前滑动，展示烤面包机和咖啡杯，再平稳转向正在使用的锅。镜头路线绕开杯柄和手臂，保持台面尺度，不穿过物体。
4–7 秒：在原片煎蛋翻动的时间段，将机位置于锅沿斜上方，轻微横移跟随煎蛋，保留原动作的速度、轨迹与落点。若翻蛋时点不同，把这一段移到对应位置，不要求人物提前或重复动作。
7–11 秒：摄影机平稳升至斜俯角，让锅、餐盘和杯子之间的相对位置清晰可见。升高过程中保持光影方向，不改变食物的成熟程度，不让原本被遮挡的物体突然消失。
11–15 秒：缓慢退回能看到两个人和早餐台面的中景，在原片动作结束后停稳。人物目光、手部交接和最终站位沿用输入视频。

保留原声与动作同步，不改变片段总时长，不替换背景，不加入文字或转场特效。不要生成两组人物、多个煎蛋或漂浮餐具。目标是让同一个早餐过程换一种观看角度，而不是重新编排早餐。
```

**看画面学什么：** 官方视频自带左右画面对照。两帧分别展示早餐场景与煎蛋动作在不同机位下的样子。
...
继续浏览[全部 9 个官方案例](docs/official-examples.md)，学习故事延长、白模和绿幕等用法。

## Seedance 2.5 适合做什么

白模是尚未添加表面材质的三维模型，用来预先安排空间与运动；绿幕用于替换拍摄背景。下表按创作任务整理常用写法。
...
九个场景涵盖电影、产品、音乐、科普、建筑与自然观察。前三条可上传配图作为起始参考，第 4–9 条可从文字开始；先选一个场景，再替换主体、动作和结尾。

**设置时长：**先在生成界面选择时长，再调整提示词的整段时间轴。较长的创意也可以拆成几个短镜头。
...
```text
20 秒、16:9，写实博物馆科普短片。场景是一张深蓝色展台，台上固定一盏小型暖白点光源、一颗哑光白色球体和一块竖直白色屏幕，三者从画面左到右依次排列。球体由细黑杆支撑，屏幕始终在球体右侧。只出现一位成年讲解员的右手，穿浅灰色长袖；不展示面孔。用真实光影演示：物体靠近光源时，屏幕上的影子变大。不要生成文字、图表或额外天体。

00:00-00:05：固定侧前方中景，一次交代灯、球、屏幕的位置。灯照亮球的左半侧，屏幕中央可见完整圆形投影。画面保持两秒，让观众辨认装置。温和的普通话旁白说：“光沿直线传播，球挡住光，留下影子。”
00:05-00:11：同一机位，不剪辑。右手握住球下方的黑杆，沿展台直线缓慢把球向灯移动约十厘米，灯和屏幕不动。屏幕上的圆形投影连续变大，圆心位置基本不变；手不经过灯与球之间，不遮挡主要投影。旁白说：“把球移近光源，影子就变大。”
00:11-00:16：右手将球沿原路线移回初始位置，影子连续缩回原来的大小；动作平稳，不产生跳帧。旁白说：“移回去，影子又变小。”
00:16-00:20：手松开支架退出画面，球、灯和屏幕静止。镜头缓慢后退少量，完整保留三者及投影，结束于能看懂装置关系的全景。

声音：清晰自然的同一位普通话旁白、手指接触支架的轻响、安静展厅的轻微空间混响；无背景音乐。灯不闪烁，球不变形，投影不能与移动方向相反，装置不能增加或消失；无字幕、标志、水印或虚构科学结论。
```
...
```text
20 秒、16:9，写实建筑导览，一镜到底。场景为一间单层庭院书屋：入口在南侧，正前方是通向庭院的直走廊，右侧是一扇已经打开的宽木门，门内是阅读室。阅读室北侧整面玻璃窗面向同一个庭院；庭院中央只有一棵小枫树和一条石板路。材料为浅色抹灰墙、橡木、浅灰石地面，阴雨刚停，柔和日光从庭院照入。无人物，无品牌，无招牌。

00:00-00:05：相机位于南侧入口内，高度约一米五，使用自然的广角视野缓慢向前移动。前方庭院的小枫树始终可见，走廊右侧开着的木门逐渐进入近景。石地面有克制的湿光反射，无大面积积水。
00:05-00:10：相机在木门前放慢，平顺向右转约九十度，穿过门洞进入阅读室。保留门框经过画面边缘的过程，明确展示走廊与室内相连；不可穿墙，不可隐藏切镜。
00:10-00:15：沿室内窗边缓慢前行，两把米色阅读椅和一张低矮圆木桌依次入画，左侧书架固定在实体墙上。焦点从近处椅背平滑移向北侧玻璃窗外的庭院，窗外仍为先前那棵枫树和石板路。
00:15-00:20：相机停在阅读室内，轻微向左转，使前景木桌、中景阅读椅、远景庭院构成清晰层次。枫树叶片轻动，檐口最后一滴雨水落下；最后两秒稳定停留，不继续转动。

声音：真实的轻微相机移动环境声、窗外雨滴、树叶轻响和远处一声鸟鸣；无脚步人物、旁白、音乐。保持房间尺度、门窗位置与家具数量一致，无墙体融化、额外出口、镜像房间、强烈鱼眼畸变、文字或水印。
```
...
```text
15 秒、16:9，写实微距自然短片，一镜到底。清晨，一只红色七星瓢虫停在横向伸展的绿色叶片上，头朝画面右侧。瓢虫有六条腿、两根短触角和左右对称的红色鞘翅，黑色斑点位置始终不变。叶片右侧靠近叶尖处只有一颗露珠。背景是柔和虚化的草地，低角度晨光从左后方透过叶片。

00:00-00:04：侧面微距固定构图，焦点落在瓢虫头部和前足，叶脉与附近露珠仍可辨认。瓢虫轻动触角，前足试探叶面，不突然移动。背景草叶只做轻微摇摆。
00:04-00:09：瓢虫沿叶脉向右缓慢爬行约一个身长，相机以同样速度平行跟随，保持主体大小和侧面观察角度。六条腿交替接触叶面，身体有重量，足部不穿透叶片。它接近露珠后停下，不触碰水珠，不振翅。
00:09-00:12：瓢虫原地停留，触角轻动。焦点从它的头部缓慢移到露珠，水珠内可见自然倒置的绿色背景折射，不出现额外动物、文字或夸张风景。叶片因微风轻轻下弯，露珠保持附着。
00:12-00:15：焦点平滑回到瓢虫头部，镜头停止移动。瓢虫抬起一只前足后再次放下，以瓢虫与露珠同框的安静画面收尾；不飞走，不切换场景。

声音：轻柔晨风、远处稀疏鸟鸣和极低的环境底噪；不把昆虫脚步夸张成敲击声，无对白、音乐或音效爆点。禁止增减腿脚、斑点漂移、翅壳软化、露珠悬空、昆虫变大、突然变焦、文字、标志或水印。
```
...
这组分镜对应[第 115 条：夜间花园动态海报](prompts/genre-social-experiments.en.md#115-night-garden-dynamic-event-poster)。九格展示**逐步累积的画面状态**：保留已经出现的东西，再增加一个新变化。
...
配套完整提示词为英文；更多材质变化、动画和社媒短片见[101–120 场景](prompts/genre-social-experiments.en.md)。
...
Seedance 2.5 可使用自然语言描述。可以从中文总索引浏览全部场景，也可以选用下列 14 份语言练习，每份包含 6 条完整提示词：
...
在 SeaImagine 选择文字或图片创作，先完成一个短镜头，再逐步延长和调整。也可以从 [Create 创作入口](https://seaimagine.com/cn/create/)继续制作参考图或探索其他作品。
...
不建议。更稳妥的写法是描述可观察的摄影属性，例如“低饱和自然光、静态对称构图、缓慢横移、克制表演”，并具体描述角色的外形与动作。
...
欢迎分享多语言提示词、实际项目、失败经验和改进方法。投稿格式见[贡献指南](CONTRIBUTING.md)。

如果这些提示词给了你灵感，欢迎 Star、分享作品，或一起补充新的场景。

## 来源与使用说明

社区作品的作者与原帖列在各案例旁；模型名称依据作者标注，发布平台和热度未独立核实。案例规格描述上传文件，原帖要求与实际文件可能不同。官方帧图取自[模型发布文章](https://seed.bytedance.com/en/blog/one-take-creation-flexible-referencing-introducing-seedance-2-5)，时间标记对应演示视频位置。

页面中的编辑改写、官方场景练习和原创提示词均供创作练习，尚未在本项目生成实测，也不代表展示视频实际使用的原始指令。封面、品牌配图和精选图例由图像工具制作；它们是视觉参考，不是 Seedance 视频效果截图。

模型与产品入口的功能、时长和画质选项以当前界面为准。使用图片、人物、音乐等素材时，请确认相应授权。可授权的代码与文本采用 [MIT 许可证](LICENSE)；外部视频、帧图、缩略图和引文仍归各自权利人所有。

[内容来源记录](docs/PROVENANCE.md) · [社区案例记录](docs/x-showcase-sources.md) · [核查记录](docs/current-source-check.md) · [模型官方页面](https://seed.bytedance.com/en/seedance2_5)

配图制作记录：[参考图与分镜](assets/IMAGE_PROMPTS.md) · [封面](assets/COVER_PROMPT.md) · [精选图例](assets/FEATURED_IMAGE_PROMPTS.md) · [纸海场景](assets/BRAND_IMAGE_PROMPT.md)
